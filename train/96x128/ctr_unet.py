...
def unet_block(input_layer, filters, kernel_size, kernel_regularizer, strides):
    conv1 = layers.Conv3D(filters=filters, kernel_size=kernel_size, kernel_regularizer=tf.keras.regularizers.L2(kernel_regularizer), strides=strides, activation="relu", padding="same")(input_layer)
    conv2 = layers.Conv3D(filters=filters, kernel_size=kernel_size, kernel_regularizer=tf.keras.regularizers.L2(kernel_regularizer), strides=strides, activation="relu", padding="same")(conv1)
    return layers.Conv3D(filters=filters, kernel_size=kernel_size, kernel_regularizer=tf.keras.regularizers.L2(kernel_regularizer), strides=strides, activation="relu", padding="same")(conv2)

def create_autoencoder(input_layer, num_blocks, filter_scale, kernel_size, kernel_regularizer, strides, max_pool):
    intermediate_layer = input_layer
    downsample = []
    for i in range(1, num_blocks+1):
        intermediate_layer = unet_block(intermediate_layer, filter_scale*(i), kernel_size, kernel_regularizer, strides)
        downsample += [intermediate_layer]
        intermediate_layer = layers.MaxPool3D(pool_size=max_pool)(intermediate_layer)
    
    enc_layer = layers.Conv3D(filters=filter_scale*num_blocks*2, kernel_size=kernel_size, kernel_regularizer=tf.keras.regularizers.L2(kernel_regularizer), strides=strides, activation="relu", padding="same")(intermediate_layer)
    
    flatten_layer = layers.Flatten()(enc_layer)
    
    # TODO: Added
    dense_layer = layers.Dense(50, activation="relu")(flatten_layer)
    
    cls_layer = layers.Dense(1, activation="sigmoid")(dense_layer)
    ...
    for i in range(1, num_blocks+1):
        intermediate_layer = layers.Conv3DTranspose(filters=filter_scale*((num_blocks+1)-i), kernel_size=kernel_size, kernel_regularizer=tf.keras.regularizers.L2(kernel_regularizer), strides=strides*2, activation="relu", padding="same")(intermediate_layer)
        intermediate_layer = layers.Concatenate()([intermediate_layer, downsample[num_blocks-i]])
        intermediate_layer = unet_block(intermediate_layer, filter_scale*((num_blocks+1)-i), kernel_size, kernel_regularizer, strides)
...
def prepare_model(inputs, use_cosine_similarity=True):
    ...
    outputs = create_autoencoder(inp, p['num_blocks'], p['filter_scale'], p['kernel_size'], p['kernel_regularizer'], p['strides'], p['max_pool'])
